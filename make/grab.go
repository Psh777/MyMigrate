...
func Grab(path string) error {
	...
	for i := 0; i < len(list); i++ {
		...
		file := fmt.Sprintf("INSERT INTO my_migrate_versions (id_version, current_version, subject) VALUES (%v, %v, '%s');", list[i].IdVersion, list[i].CurrentVersion, list[i].Subject)
